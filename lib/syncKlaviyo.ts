...
export async function runKlaviyoSync(apiKey: string) {
  ...
  try {
    ...
    const { data: orderStats } = await service.from('orders').select('total_price').eq('store_id', STORE_ID).eq('financial_status', 'paid').neq('test', true)
  ...
  try {
    ...
    const { data: shopifyRevRows } = await service.from('orders').select('total_price').eq('store_id', STORE_ID).eq('financial_status', 'paid').neq('test', true)
