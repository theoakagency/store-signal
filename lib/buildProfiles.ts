...
export async function runProfileBatch(batch: number) {
  ...
  let from = 0
  while (true) {
    const { data, error } = await service.from('orders').select('email, total_price, processed_at, created_at').eq('store_id', STORE_ID).eq('financial_status', 'paid').neq('test', true).range(from, from + 999)
  ...
  for (const sub of allSubs) {
    ...
    if (!subsByEmail.has(email) || sub.status === 'active') {
      ...
      subsByEmail.set(email, { status: sub.status ?? '', interval: `Every ${freq} ${unit}`, mrr: Math.round(mrr * 100) / 100 })
    }
  }
  ...
  for (const lc of allLoyalty) {
    ...
    loyaltyByEmail.set(lc.email.toLowerCase(), { tier: lc.tier ?? '', balance: lc.points_balance ?? 0, spent: lc.points_spent_total ?? 0 })
  }
  ...
  // ── LTV percentile thresholds ─────────────────────────────────────────────
  const allRevenues = Array.from(byEmail.values()).map((c) => c.amounts.reduce((s, v) => s + v, 0)).sort((a, b) => a - b)
  ...
  if (batchEmails.length === 0) {
    return { ok: true, batch, totalBatches, totalEmails, processed: 0, upserted: 0 }
  }
  ...
  for (const email of batchEmails) {
    ...
    const daysSinceLast = lastOrderAt ? Math.floor((now.getTime() - sortedTs[sortedTs.length - 1]) / 86400000) : null
  ...
  for (let i = 0; i < profiles.length; i += CHUNK) {
    const { error } = await service.from('customer_profiles').upsert(profiles.slice(i, i + CHUNK), { onConflict: 'tenant_id,email' })
    if (error) throw new Error(`Profile upsert failed: ${error.message}`)
  }

  // ── Final batch: compute overlap cache + segment/LTV stats ───────────────
  ...
  return { ok: true, batch, totalBatches, totalEmails, processed: profiles.length, upserted: profiles.length }
}
