/**
 * Core Recharge sync logic, extracted so it can be called from both the
 * manual POST /api/recharge/sync route and the automated cron job.
 */
...
export async function runRechargeSync(apiToken: string): Promise<{
  ok: true
  synced: { subscriptions: number }
  metrics: { active_subscribers: number; mrr: number; arr: number; churn_rate_30d: number }
}> {
  ...
  // Suppress unused variable warning — recentOrders is fetched for side-effect
  void recentOrders
