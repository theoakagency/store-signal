...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('orders').select('total_price, currency, processed_at').eq('financial_status', 'paid').neq('test', true).is('cancelled_at', null).gte('processed_at', thirtyDaysAgo.toISOString()),
    supabase.from('orders').select('total_price').eq('financial_status', 'paid').neq('test', true).is('cancelled_at', null).gte('processed_at', sixtyDaysAgo.toISOString()).lt('processed_at', thirtyDaysAgo.toISOString()),
    supabase.from('orders').select('total_price, currency').eq('financial_status', 'paid').neq('test', true).is('cancelled_at', null).gte('processed_at', sevenDaysAgo.toISOString()),
    supabase.from('orders').select('total_price').eq('financial_status', 'paid').neq('test', true).is('cancelled_at', null).gte('processed_at', fourteenDaysAgo.toISOString()).lt('processed_at', sevenDaysAgo.toISOString()),
