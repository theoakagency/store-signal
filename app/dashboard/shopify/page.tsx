...
export default async function ShopifyPage() {
  ...
  const [
    ...
    supabase.from('orders').select('total_price, email, processed_at').eq('store_id', STORE_ID).eq('financial_status', 'paid').neq('test', true).is('cancelled_at', null).gte('processed_at', thirtyDaysAgo.toISOString()),
    supabase.from('orders').select('total_price').eq('store_id', STORE_ID).eq('financial_status', 'paid').neq('test', true).is('cancelled_at', null).gte('processed_at', sixtyDaysAgo.toISOString()).lt('processed_at', thirtyDaysAgo.toISOString()),
