...
export async function POST(req: NextRequest) {
  ...
  const [
    ...
    service.from('orders').select('total_price, created_at').eq('tenant_id', TENANT_ID).eq('financial_status', 'paid').neq('test', true).is('cancelled_at', null).gte('created_at', new Date(Date.now() - 30 * 86400000).toISOString()),
