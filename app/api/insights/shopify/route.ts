...
export async function POST(_req: NextRequest) {
  ...
  const [
    ...
    service.from('orders').select('total_price, financial_status, fulfillment_status, processed_at').eq('store_id', STORE_ID).eq('financial_status', 'paid').neq('test', true).is('cancelled_at', null).gte('processed_at', thirtyDaysAgo.toISOString()),
    service.from('orders').select('total_price').eq('store_id', STORE_ID).eq('financial_status', 'paid').neq('test', true).is('cancelled_at', null).gte('processed_at', sixtyDaysAgo.toISOString()).lt('processed_at', thirtyDaysAgo.toISOString()),
